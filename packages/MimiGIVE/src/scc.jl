...
function post_trial_func(mcs::SimulationInstance, trialnum::Int, ntimesteps::Int, tup)
    ...
    if options.compute_sectoral_values
        ...
        smoke_mds               = (marginal[:DamageAggregator, :smoke_damage] .- base[:DamageAggregator, :smoke_damage]) .* gas_units_multiplier 
        ...
        if options.compute_domestic_values
            ...
            smoke_mds_domestic               = (marginal[:DamageAggregator, :smoke_damage_domestic] .- base[:DamageAggregator, :smoke_damage_domestic]) .* gas_units_multiplier 
        end
    end
    ...
    if options.save_md
        ...
        if options.compute_sectoral_values
            ...
            md_values[(region=:globe, sector=:smoke)][trialnum, :]                = smoke_mds[_damages_idxs]
        end
        ...
        if options.compute_domestic_values
            ...
            if options.compute_sectoral_values
                ...
                md_values[(region=:domestic, sector=:smoke)][trialnum, :]                = smoke_mds_domestic[_damages_idxs]
            end
        end
    end
    ...
    for dr in discount_rates
        ...
        if options.compute_sectoral_values
            ...
            scc = sum(df .* smoke_mds[year_index:last_year_index])
            scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta)][trialnum] = scc
            ...
            if options.certainty_equivalent
                ...
                intermediate_ce_scc = sum(df_ce .* smoke_mds[year_index:last_year_index])
                intermediate_ce_scc_values[(region=:globe, sector=:smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta)][trialnum] = intermediate_ce_scc
            end
            ...
            if options.compute_domestic_values
                ...
                scc = sum(df .* smoke_mds_domestic[year_index:last_year_index])
                scc_values[(region=:domestic, sector= :smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta)][trialnum] = scc
                ...
                if options.certainty_equivalent
                    ...
                    intermediate_ce_scc = sum(df_ce .* smoke_mds_domestic[year_index:last_year_index])
                    intermediate_ce_scc_values[(region=:domestic, sector= :smoke, dr_label=dr.label, prtp=dr.prtp, eta=dr.eta)][trialnum] = intermediate_ce_scc
                end
            end
        end
    end
...
function _compute_scc_mcs(mm::MarginalModel, 
    ...
    sectors = compute_sectoral_values ? [:total,  :cromar_mortality, :agriculture, :energy, :slr, :smoke] : [:total]
